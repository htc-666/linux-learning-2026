...
### 7. sigprocmask 屏蔽 SIGKILL/SIGSTOP 无效
...
8. sigprocmask + pause 有时间窗口
sigprocmask(SIG_SETMASK, &mask, &old);
// ⚠ 这里信号可能溜走
pause();
修复: 用 sigsuspend(&mask) 原子操作。

9. 实时信号 handler 必须用 sa_sigaction
sa.sa_handler = handler;     // ❌ 拿不到 si_value 等数据
sa.sa_flags = SA_SIGINFO;
修复:

sa.sa_sigaction = handler;   // ✓ 配合 SA_SIGINFO
10. abort 必死,handler 拦不住
sigaction(SIGABRT, &sa, NULL);   // 注册了 handler
abort();                          // 仍然死(handler 跑完后必杀)
abort 设计就是"保证死",和普通 kill -6 不同。
